...
export default function CastingMastersToolbar({ qp }: { qp: CastingMastersQP }) {
  const [state, setState] = useQueryStates(castingMastersParsers, {
    shallow: false,
  });
  ...
  const activeFilters =
    (qp.q ? 1 : 0) + (qp.status !== "ALL" ? 1 : 0) + (qp.trash !== "EXCLUDE" ? 1 : 0);
    ...
    <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      <div className="flex flex-1 flex-col gap-3 md:flex-row md:items-center">
        <Input
          ...
          placeholder="Search casting name / code / drawing / HSN"
        />
      ...
      <Button asChild>
        <Link href="/dashboard/casting-masters/new">
          <Plus />
          New Casting
        </Link>
      </Button>
