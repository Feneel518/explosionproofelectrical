...
const ProductVariantForm: FC<ProductVariantFormProps> = ({
  ...
  const isDuplicateMode = mode === "create" && !!variant;
  const duplicatedVariantName = variant?.variant
    ? `${variant.variant} Copy`
    : "";
  const initialVariantName = isDuplicateMode
    ? duplicatedVariantName
    : variant?.variant ?? "";

  const form = useForm<ProductVariantSchemaRequest>({
    resolver: zodResolver(ProductVariantSchema),
    defaultValues: {
      id: isEditMode ? (variant?.id ?? undefined) : undefined,
      productId,
      ...
      drawings: variant?.drawings
        ? variant.drawings.map((dwg) => {
            return {
              id: isEditMode ? (dwg.id ?? undefined) : undefined,
              kind: dwg.kind,
              title: dwg.title ?? "",
              url: dwg.url ?? "",
            };
      ...
      images: variant?.image
        ? variant.image.map((img) => {
            return {
              id: isEditMode ? (img.id ?? undefined) : undefined,
              kind: img.kind,
              title: img.title ?? "",
              url: img.url ?? "",
            };
      ...
      size: variant?.size ?? "",
      sku:
        isEditMode || !initialVariantName
          ? variant?.sku ?? ""
          : buildBaseSku(productName, initialVariantName),
      status: isEditMode ? (variant?.status ?? "ACTIVE") : "ACTIVE",
      ...
      component: variant?.component
        ? variant.component.map((comp) => {
            return {
              id: isEditMode ? (comp.id ?? undefined) : undefined,
              item: comp.item ?? "",
              unit: comp.unit ?? "",
            };
          })
  ...
  useEffect(() => {
    if (mode === "create" && variantName) {
      form.setValue("sku", buildBaseSku(productName, variantName));
    }
  }, [form, mode, productName, variantName]);
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ">
            ...
            {[
              ...
              },
              {
                name: "terminals",
                label: "Terminals",
                placeholder: "IIA, IIB and IIC as per IS/IEC:60079-1",
              },
              {
                name: "gasket",
                label: "Gasket",
                ...
                <FormItem>
                  <FormControl>
                    <FileUpload
                      ...
                      label="Upload Images"
                      hint="PNG or JPG. Recommended: 512x512."
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
