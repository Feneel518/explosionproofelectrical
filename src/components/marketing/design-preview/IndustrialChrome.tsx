...
export function IndustrialHeader() {
  ...
  const links = [["CATALOG", "/catalog"], ["KNOWLEDGE", "/knowledge-hub"], ["ENGINEERING", "/engineering"], ["OUR STORY", "/story"], ["LET’S TALK", "/contact"]] as const;
  ...
  return <header className={styles.header}>
    ...
    {open && <nav id="mobile-navigation" className={styles.mobileNav} aria-label="Mobile navigation">{links.map(([label, href]) => <Link key={href} href={href} aria-current={isActive(href) ? "page" : undefined} onClick={() => setOpen(false)}>{label}<ArrowUpRight size={18} /></Link>)}</nav>}
  </header>;
}
...
export function IndustrialFooter() {
  return <footer className={styles.footer}><div className={styles.footerMain}>
    ...
    <nav aria-label="Footer navigation"><Link href="/catalog">Catalog <ArrowUpRight size={14} /></Link><Link href="/engineering">Engineering <ArrowUpRight size={14} /></Link><Link href="/story">Our story <ArrowUpRight size={14} /></Link><Link href="/contact">Contact <ArrowUpRight size={14} /></Link></nav>
