...
export function IndustrialProduct({ product, related }: { product: CatalogProductDetail; related: CatalogProductCard[] }) {
  ...
  return <>
    <nav className={styles.breadcrumb} aria-label="Breadcrumb"><Link href="/catalog"><ArrowLeft size={15} /> Catalog</Link><span>/</span><Link href={`/catalog?cat=${product.categorySlug}`}>{product.cat}</Link><span>/</span><span aria-current="page">{product.name}</span></nav>
