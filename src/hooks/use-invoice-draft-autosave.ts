...
import { saveInvoiceDraftSnapshotAction } from "@/lib/actions/dashboard/sales/invoice/saveInvoiceDraftSnapshotAction";
...
export function useInvoiceDraftAutosave({
  ...
  const runSave = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await saveInvoiceDraftSnapshotAction({
        invoiceId,
        draft,
  ...
  }, [enabled, getDraft, invoiceId, version]);
