import { saveMaterialIssueDraftSnapshotAction } from "@/lib/actions/dashboard/manufacturing/material-issue/saveMaterialIssueDraftSnapshotAction";
import { MaterialIssueDraftData } from "@/lib/actions/dashboard/manufacturing/material-issue/createDraftMaterialIssueAction";
import React from "react";

export function useMaterialIssueDraftAutosave({
  getDraft,
  initialVersion,
  materialIssueId,
  debounceMs = 1200,
  ...
  getDraft: () => MaterialIssueDraftData;
  enabled?: boolean;
  debounceMs?: number;
}) {
  ...
  const runSave = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await saveMaterialIssueDraftSnapshotAction({
        materialIssueId,
        draft,
  ...
  }, [enabled, getDraft, materialIssueId, version]);
