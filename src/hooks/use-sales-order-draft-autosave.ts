...
import { saveSalesOrderDraftSnapshotAction } from "@/lib/actions/dashboard/sales/order/saveSalesOrderDraftSnapshotAction";
import React from "react";

export function useSalesOrderDraftAutosave({
  getDraft,
  initialVersion,
  salesOrderId,
  debounceMs = 1200,
  ...
  const runSave = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await saveSalesOrderDraftSnapshotAction({
        salesOrderId,
        draft,
  ...
  }, [enabled, getDraft, salesOrderId, version]);
